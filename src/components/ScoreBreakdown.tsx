        ...
        return (
          <div key={c.factor}>
            <div className="mb-1 flex items-center justify-between text-xs">
              ...
              <span className={clsx('tabular-nums font-semibold', positive ? 'text-emerald-600' : 'text-rose-600')}>
                {positive ? '+' : ''}
                {pct.toFixed(1)}%
              </span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-slate-100">
              <div className={clsx('h-full rounded-full', positive ? 'bg-brand-500' : 'bg-rose-400')} style={{ width }} />
            </div>
