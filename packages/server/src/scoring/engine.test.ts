...
import { DEFAULT_SCORING_WEIGHTS, DEFAULT_SCORING_PARAMS } from '@ais/shared';
import { rankGameRequests, scoreRequest } from './engine.js';
import type { ResolvedScoringConfig, ScoringGameInput, ScoringRequestInput, ScoringContext } from './types.js';
...
const game: ScoringGameInput = { id: 1, gameDate: NOW + 21 * DAY, premiumScore: 0.8 };

function req(partial: Partial<ScoringRequestInput> & { requestId: number }): ScoringRequestInput {
  return {
...
describe('scoreRequest', () => {
  it('is deterministic and produces a breakdown that sums to the final score', () => {
    ...
    const r = req({ requestId: 1, tier: 'platinum', salesOpportunityUsd: 50000 });
  ...
  it('ranks a high-value, high-opportunity customer above a low-tier one', () => {
    const high = rankGameRequests(
      [req({ requestId: 1, tier: 'platinum', salesOpportunityUsd: 50000, attendedCount: 9, awardedCount: 10 })],
      game,
      config,
      5,
      NOW
    ).ranked[0].finalScore;
    const low = rankGameRequests([req({ requestId: 2, tier: 'prospect' })], game, config, 5, NOW).ranked[0].finalScore;
...
describe('rankGameRequests', () => {
  it('allocates against inventory: awards up to capacity, waitlists the rest', () => {
    const requests = [1, 2, 3, 4].map((id) => req({ requestId: id, quantity: 2 }));
  ...
  it('breaks ties deterministically by tier then earlier request', () => {
    ...
    const a = req({ requestId: 10, tier: 'gold', createdAt: NOW });
    const b = req({ requestId: 11, tier: 'bronze', createdAt: NOW });
    const result = rankGameRequests([b, a], game, config, 5, NOW);
    expect(result.ranked[0].requestId).toBe(10);
  });

  it('rewards fairness: a never-awarded requester outscores an identical recently-awarded one', () => {
    const fresh = req({ requestId: 20, lastTicketDate: null });
    const recent = req({ requestId: 21, lastTicketDate: NOW - 2 * DAY });
    const result = rankGameRequests([recent, fresh], game, config, 5, NOW);
    const freshScore = result.ranked.find((r) => r.requestId === 20)!.finalScore;
    const recentScore = result.ranked.find((r) => r.requestId === 21)!.finalScore;
